...
fun main() {
    ...
    val differencesCounts: Map<Int, Counter> = JoltageAdapterList(
        readFileLineByLineToInt(2020, 10).toList().map { JoltageAdapter(it) }).getDifferencesCounts()
    ...
    println( JoltageAdapterList(
        readFileLineByLineToInt(2020, 10).toList().map { JoltageAdapter(it) }).getPossibilitiesCount())
}
