...
fun main() {
    ...
    println(getArea(readFileLineByLineToText("2020_03.txt").toList()).walkthrough(0,0,3,1).filter { it.tree }.count())

    printDay(2)
    print("Trees encountered: ")
    println(
        1L
        *   getArea(readFileLineByLineToText("2020_03.txt").toList()).walkthrough(0,0,1,1).filter { it.tree }.count()
        *   getArea(readFileLineByLineToText("2020_03.txt").toList()).walkthrough(0,0,3,1).filter { it.tree }.count()
        *   getArea(readFileLineByLineToText("2020_03.txt").toList()).walkthrough(0,0,5,1).filter { it.tree }.count()
        *   getArea(readFileLineByLineToText("2020_03.txt").toList()).walkthrough(0,0,7,1).filter { it.tree }.count()
        *   getArea(readFileLineByLineToText("2020_03.txt").toList()).walkthrough(0,0,1,2).filter { it.tree }.count()
    )
}
