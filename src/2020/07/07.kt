...
fun main() {
    ...
    println(canContainBag("shiny gold", readFileLineByLineToText("2020_07.txt").map { Rule(it) }.toList()).count())

    printDay(2)
    print("Amount of bags a shiny gold bag has to contain: ")
    println(getBagCount("shiny gold", readFileLineByLineToText("2020_07.txt").map { Rule(it) }.toList()) - 1)
}
